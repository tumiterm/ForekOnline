        ...
        public async Task<IActionResult> StudentDocuments(StudentAttachment attachment)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (file != null)
                {
                    ...
                    if (rc > 0)
                    {
                        _notify.Success("File uploaded successfully", 5);

                        Content(Helper.ShowNotification("Success", "Your record was saved!", "success"));

                        return RedirectToAction("StudentDocuments", new { StudentNumber = attachment.StudentNumber, StudentId = attachment.StudentId });
                    }
                    else
                    {
                        _notify.Error("Error: Unable to upload file!", 5);
                    }
                }
                else
                {
            ...
            {
                _notify.Error("Error: A file is required before submission!", 5);
            }
